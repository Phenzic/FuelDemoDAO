...
const CONTRACT_ID = "0xfd6afe13d5993ad373984e74e305427f2e86f852050c135d7e88ce5afcaaa376"; // Replace with your contract ID
...
const Home: React.FC = () => {
  ...
  const fetchProposals = async (contractInstance: ContractAbi, proposalId: string) => {
    try {
      // Fetch the proposal from the contract
      const proposal = await contractInstance.functions.proposal(proposalId).call();
  ...
  const createProposal = async () => {
    try {
      ...
      contract.functions.create_proposal(acceptancePercentage, duration, proposalTransaction);
          ...
          <>
            ...
            {activeTab === 'create' && (
              <>
                ...
                <input type="number" value={acceptancePercentage} onChange={(e) => setAcceptancePercentage(parseInt(e.target.value))} />
                ...
                <input
                  ...
                  onChange={(e) => setProposalTransaction({ ...proposalTransaction, amount: parseInt(e.target.value) })}
                />
            ...
            {activeTab === 'vote' && (
              <>
                ...
                <div>
                  ...
                  <input type="text" value={proposalIdInput} onChange={(e) => setProposalIdInput(e.target.value)} />
                  <button onClick={() => contract && fetchProposals(contract, proposalIdInput)}>Fetch Proposal</button>
                </div>
                {proposals.map((proposal: any) => ( // Adjust the type of proposal according to your data structure
                  <div key={proposal.id} style={{ marginBottom: '10px' }}>
                    <div>{proposal.text}</div>
                    <button onClick={() => voteOnProposal(proposal.id)} style={styles.button}>
                      Vote
                    </button>
                  </div>
                ))}
